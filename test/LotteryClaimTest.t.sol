...
contract LotteryClaimTest is Test {
    ...
    function fulfill(uint32 n1, uint32 n2, uint32 n3, uint32 n4, uint32 n5, uint8 s) internal {
        vm.mockCall(
    ...
    function testSinglePlayer_fullTicket() public {
        ...
        assertEq(token.balanceOf(address(round)), ticketPrice * 9);
        assertEq(token.balanceOf(address(lottery)), ticketPrice * lottery.MAX_SHARES());
        vm.startPrank(alice);
        lottery.claim(tokenId);
        vm.expectRevert(bytes("LT09"));
        lottery.claim(tokenId);
        vm.stopPrank();
        assertEq(token.balanceOf(address(alice)), ticketPrice * (33_334 + 13_334 + 13_334));
